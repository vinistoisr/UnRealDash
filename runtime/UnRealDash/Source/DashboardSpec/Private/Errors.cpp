...
ErrorText::ErrorText(std::string_view text) : text_(new(std::nothrow) char[text.size() + 1]), length_(text.size()) {
    if (!text_) {
        length_ = 0;
        return;
    }
...
const char *CodeName(ErrorCode code) {
    switch (code) {
    case ErrorCode::E_PKG_ASSET_UNREADABLE:
        return "E_PKG_ASSET_UNREADABLE";
