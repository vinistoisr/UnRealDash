...
Error ArchiveGate(const std::filesystem::path &path) {
    ...
    if (Little(tail, eocd + 4, 2) || Little(tail, eocd + 6, 2))
        return Io(path.generic_string());
    const bool requires_zip64 =
        count == 0xffff || disk_count == 0xffff || central_size == 0xffffffff || central_offset == 0xffffffff;
...
struct LoadedPackage::Storage {
    struct AssetEntry {
        std::string name;
        std::filesystem::path file;
        std::uint64_t expected{};
        mutable std::unique_ptr<std::uint8_t[]> bytes;
...
LoadedPackage::LoadedPackage(LoadedPackage &&other) noexcept : storage_(other.storage_) { other.storage_ = nullptr; }
LoadedPackage &LoadedPackage::operator=(LoadedPackage &&other) noexcept {
    if (this != &other) {
        delete storage_;
...
Error LoadedPackage::Asset(std::string_view name, std::span<const std::uint8_t> &out) const {
    out = {};
    if (!storage_)
        return Fail(ErrorCode::E_PKG_ASSET_NOT_IN_PACKAGE, "", name);
    for (const auto &asset : storage_->assets) {
        if (asset.name != name)
            continue;
        if (!asset.bytes) {
            const auto pointer = Join("/entries", name);
            // Lazy reads recheck the admitted path, including every ancestor. A changed
            // package must not turn this cache into a path around the link gate.
            std::error_code ec;
            for (auto file = asset.file;; file = file.parent_path()) {
                const auto status = std::filesystem::symlink_status(file, ec);
                if (ec)
                    return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
                if (std::filesystem::is_symlink(status))
                    return Fail(ErrorCode::E_PKG_PATH_LINK, "", name);
                if (file == storage_->root)
                    break;
                if (file == file.parent_path())
                    return Fail(ErrorCode::E_PKG_PATH_LINK, "", name);
            }
            const auto links = std::filesystem::hard_link_count(asset.file, ec);
            if (ec)
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            if (links > 1)
                return Fail(ErrorCode::E_PKG_PATH_LINK, "", name);
            const auto size = std::filesystem::file_size(asset.file, ec);
            if (ec)
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            if (size > limits::asset_size)
                return Fail(ErrorCode::E_PKG_ASSET_SIZE, pointer, name);
            if (size != asset.expected)
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            auto bytes =
                std::unique_ptr<std::uint8_t[]>(new (std::nothrow) std::uint8_t[static_cast<std::size_t>(size) + 1]);
            if (!bytes)
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            std::size_t got = 0;
            if (!ReadBounded(asset.file, {bytes.get(), static_cast<std::size_t>(size) + 1}, got))
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            if (got > limits::asset_size)
                return Fail(ErrorCode::E_PKG_ASSET_SIZE, pointer, name);
            if (got != size)
                return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
            asset.size = got;
        ...
        out = {asset.bytes.get(), asset.size};
        return {};
    }
    return Fail(ErrorCode::E_PKG_ASSET_NOT_IN_PACKAGE, "", name);
}
Error PackageReader::Read(std::string_view path, Profile profile) const { return Validate(path, profile, nullptr); }
Error PackageReader::Load(std::string_view path, Profile profile, LoadedPackage &out) const {
    return Validate(path, profile, &out);
}
Error PackageReader::Validate(std::string_view path, Profile profile, LoadedPackage *out) const {
    std::error_code ec;
    ...
    if (directory) {
        ...
        for (mz_uint i = 0; i < files; ++i) {
            ...
            const bool regular = (mode == 0100000 || (mode == 0 && !dos_directory && !dos_reparse)) &&
                                 !stat.m_is_directory && !dos_directory;
    ...
    if (out) {
        ...
        for (const auto &[name, entry] : entries) {
            if (entry.directory || JsonPath(name))
                continue;
            LoadedPackage::Storage::AssetEntry asset;
            asset.name = name;
            asset.expected = entry.bytes;
            if (directory) {
                asset.file = std::filesystem::absolute(entry.file, ec).lexically_normal();
                if (ec)
                    return Io(name);
            } else {
                const auto pointer = Join("/entries", name);
                // One extra byte detects a dishonest expanded size. Neither allocation
                // nor extraction uses a throwing container for the asset payload.
                const auto capacity = static_cast<std::size_t>(entry.bytes) + 1;
                asset.bytes.reset(new (std::nothrow) std::uint8_t[capacity]);
                if (!asset.bytes)
                    return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
                auto *iterator = mz_zip_reader_extract_iter_new(&archive.zip, entry.index, 0);
                if (!iterator)
                    return Fail(ErrorCode::E_PKG_ASSET_UNREADABLE, pointer, name);
                while (asset.size < capacity) {
                    const auto got = mz_zip_reader_extract_iter_read(iterator, asset.bytes.get() + asset.size,
                                                                     capacity - asset.size);
                    asset.size += got;
                    if (!got || iterator->status < 0)
                        break;
                }
                const bool finished = iterator->status == TINFL_STATUS_DONE && iterator->out_blk_remain == 0;
                const bool valid = mz_zip_reader_extract_iter_free(iterator) != 0;
                if (asset.size > limits::asset_size)
                    return Fail(ErrorCode::E_PKG_ASSET_SIZE, pointer, name);
                if (!finished || !valid || asset.size != entry.bytes)
                    return Io(name);
            }
            storage.assets.push_back(std::move(asset));
        }
        for (const auto &asset : storage.assets)
            storage.names.push_back(asset.name);
        storage.document.Data().json.Swap(bundle.Data().json);
        *out = std::move(loaded);
    }
    return {};
}
