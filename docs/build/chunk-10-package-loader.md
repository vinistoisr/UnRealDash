...
- In `DashboardSpec.Build.cs`, add each of the five schema files as a `RuntimeDependency` whose **source** is `packages/dashboard-spec/schema/<name>.schema.json` and whose **staged destination is pinned** to `$(ProjectDir)/Schema/<name>.schema.json`, with `StagedFileType.NonUFS` so packaging puts them on the filesystem rather than inside the `.pak`.
  ...
  **This did not deliver on Android when the APK was installed directly, 2026-09-17.** The `RuntimeDependency` stages correctly to `Saved/StagedBuilds/Android/UnRealDash/Schema/`, but with `bPackageDataInsideApk=True` the APK carries only the cooked UFS data, as `assets/main.obb.png`. A listing of the built APK finds **zero** schema entries, the generated installer has no push steps for them, the directory never appears on the device, and every package is rejected with `E_SCHEMA` "cannot read schema dashboard". Pushing the five files by hand makes the same fixture pass, so the loader is fine and only the delivery is broken. Note that UAT's own deploy step does push non-UFS files from a manifest; whether `RunUAT -deploy` would fix this is untested, and the gate installs an APK directly, which is the case that must work. See `docs/reports/2026-09-17-device-package-gate.md` for the evidence and three candidate fixes; the choice is the owner's because one of them changes `Validator`'s constructor. Do not let the destination default: the default mirrors the source path, and the resolver below would look in the right place and find nothing. That failure is invisible on Windows, where the repository copy exists anyway, and only appears on device.
