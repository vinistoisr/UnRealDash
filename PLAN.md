...
Gate: two gates, one per part. Base: the workflow is green on both hosted jobs and the test step prints a non-zero assertion count. ThreadSanitizer job: green on `ubuntu-latest` with zero data races reported and the 2.13 stress test's own assertion count non-zero. In both parts a `git config --get lfs.*` probe in the job log confirms no LFS objects were fetched.
...
Deliverable: `scripts/build.ps1`, `package-windows.ps1`, `package-android.ps1`, `package-linux.ps1`, `deploy-deck.ps1`, `capture-metrics.ps1`, each of which calls `doctor.ps1` first and refuses to run if it fails. `build.ps1`, `package-windows.ps1`, `package-android.ps1` and `package-linux.ps1` call `-Profile toolchain`. `deploy-deck.ps1` always calls `-Profile device`, because its whole job is to reach the device. `capture-metrics.ps1` selects its profile from its target rather than using a fixed one: a Windows capture and the 6.6b desktop soak select `toolchain`, a device capture selects `device`. A workstation measurement therefore never depends on the head unit being awake.
Gate: each script run with `-WhatIf` prints the exact UAT command line it would execute and exits 0; each script run with a deliberately failed doctor exits non-zero without invoking UAT; `build.ps1`, `package-windows.ps1` and `package-linux.ps1` each complete a `-WhatIf` run with the head unit powered off; and `capture-metrics.ps1 -Target windows` completes a `-WhatIf` run with the head unit powered off while `capture-metrics.ps1 -Target device` exits non-zero in the same conditions, with the profile it selected printed in both runs.
...
**Rules are driven by time as well as by arrivals.** Evaluating only when a sample arrives cannot expire a `hold_last` timer and cannot raise a stale-input warning during total silence, which is precisely the condition that needs a warning. Every rule is evaluated on each sample arrival and also on an acquisition-side timer tick, a chosen default of 10 Hz, using the same injectable clock as 2.2. A rule whose inputs have all gone silent therefore transitions to `stale` handling on a tick, with no traffic at all.
...
The two mechanisms need different answers, which is why they are one task. A heartbeat is not a measurement, so it must not touch a signal at all and the signal must go stale. A held frame **is** a measurement, of unknown age, so it keeps the signal `valid` and tells the operator what it does not know through `age_evidence`. Treating the second like the first was the Round 1 error: it would mark an equilibrium reading as a transport failure.
...
Deliverable: a doctest-driven stress harness in `packages/signal-core/tests/` running a real writer thread and a real reader thread against the 2.2 registry, with a deterministic seed and a fixed iteration count rather than a wall-clock duration, so the same run is reproducible on a slow hosted runner. It covers four scenarios: sustained publication with a continuously swapping reader; a reader that stalls while holding a snapshot across 1000 publications; a generation change mid-stream with older-generation samples still in flight; and a bounded queue driven above its capacity with the drop counter checked.
Gate: under a normal build, the held-snapshot scenario asserts the reader's snapshot is bit-identical before and after the 1000 publications and the writer's blocked count is zero; the generation scenario asserts no older-generation sample is applied and each is counted; the queue scenario asserts depth never exceeds the bound and the drop count equals publications minus consumptions; the sustained scenario asserts no growth in resident allocation over the run. Under `-fsanitize=thread` on `ubuntu-latest`, the whole harness reports zero data races. Both runs print their assertion count.
...
The gate is in two halves, because they need different toolchains and become available at different times.
Gate, Win64 half: `packages/signal-core`'s CMake build compiles the same files with exceptions and RTTI off on `windows-latest` and `ubuntu-latest` and its doctest suite passes; a grep across `runtime/UnRealDash/Source/SignalCore/` finds no Unreal header include outside the excluded module-registration file; UBT builds the module for Win64; and an in-engine smoke commandlet runs a representative subset of the signal-core checks in-process on Win64, covering freshness transitions, unit normalization and rule evaluation, plus the 2.13 stress harness per 4.3, and prints the assertion count and the failure count. The gate is the assertion count matching the CMake run's count for the same subset with zero failures, not a version string. This half proves the sources are engine-independent and that the two compilers agree, which is R-N's test.
Gate, Android and Linux half: UBT builds the same module for Android ARM64 and for Linux x86-64. This half proves only that Epic's cross-compilation toolchains accept the sources, and it needs 0.5 and 0.6 installed, so it cannot run on the Win64 half's schedule. Splitting them is what stops a Win64-ready module from being reported as blocked on an Android SDK it does not use.
...
The threading contract is explicit. The acquisition side owns the registry and is its only writer. Every sample that arrives is evaluated against the rules on the acquisition side, at sample rate, before any coalescing for display: a warning condition that exists for one sample at 200 Hz latches and is not lost because the renderer only looked 16 ms later. Rules also evaluate on the 2.5 timer tick, so a warning can be raised during silence and a `hold_last` timer can expire with no traffic at all. Latched warning state clears on an explicit acknowledge call or on a generation change, per 2.5. The bounded queue and its drop policy sit between acquisition and display, not between the wire and rule evaluation, so dropping for display cost never erases a rule transition.
...
Deliverable also includes the in-engine reuse of 2.13: the 4.2 smoke commandlet runs 2.13's stress harness in-process on Win64 against the real game-thread acquire, so the same scenarios that passed under CMake and ThreadSanitizer are re-run against the engine's threading rather than being re-implemented against it.
...
The event log carries **three** record types, each with its own declared column schema:
1. **Per-frame rows.** Frame index, frame start, frame time, present timestamp, missed-frame flag, resident memory, texture memory.
2. **Per-sample receive rows and present rows, recorded separately and correlated by a sample identifier.** A receive row is written when a sample is published: sample id, signal id, receive timestamp, sequence, connection generation, quality, `age_evidence`. A present row is written when a sample is first displayed: sample id, the frame index and present timestamp of that frame, and an interpolation attribution field naming the sample id the displayed value was derived from when the displayed value came from the 2.6 interpolator rather than directly from a sample.
3. **A 1 Hz sampled stream.** Temperature from the per-platform source named in 6.5, resident memory, texture memory and open handle count, one row per second for the life of the process, independent of frame rate. The thermal run in 6.6 and the desktop soak in 6.6b read this stream, not the per-frame rows: a run that stalls or drops to 5 fps must still produce a temperature and memory trace, and a per-frame row cannot provide one.
...
**Every published sample ends in exactly one of three presentation classifications**, recorded rather than inferred: `presented` (a present row exists for it), `coalesced` (a newer sample for the same signal superseded it before any frame displayed it), or `pending` (the run ended before it was either presented or superseded). The Round 1 gate that required a present timestamp for every published sample was unsatisfiable: 4.3 deliberately coalesces for display, so most samples at 200 Hz into 60 Hz never reach a frame. Writing a timestamp for them anyway would fabricate latency, and dropping them silently would hide how many updates the display never showed. Classifying them is what makes the coalescing count a reported number instead of a gap.
Timestamp definitions, so the numbers mean the same thing on both platforms: **receive-to-present** is the sample's receive timestamp subtracted from the present timestamp of the frame that displayed it, both taken from the same monotonic clock in the same process, and is defined only over `presented` samples. **Launch-to-first-usable-value** is process start to a logged first-usable-frame event, where first-usable means the first present callback for a frame in which at least one bound signal rendered a `valid` value.
The cold-start clock domain is stated because the two endpoints must come from one clock or the difference means nothing. The procedure is force-stop the app, clear it from recents, then launch. The primary measurement of process start is read **in-process**: `/proc/self/stat` field 22 on Android and Linux, converted from boot ticks to the process's monotonic clock, and `GetProcessTimes` on Windows, converted the same way. Both endpoints are then on the clock the first-usable-frame event already uses. `am start -W` is recorded as **auxiliary evidence only**, alongside the launch state it reports (`COLD`, `WARM` or `HOT`), because its output is a set of durations measured by the activity manager rather than an absolute timestamp in the app's clock domain, and combining a duration with an in-process timestamp would produce a number with no defined meaning. Where `am start -W` and the in-process figure disagree, the in-process figure is the measurement and the difference is recorded.
Gate: a 60-second run writes an event log whose per-frame row count matches the frame count within 1 percent, whose receive rows cover every published sample, and whose sampled stream has one row per second within 1 percent of the elapsed seconds; every published sample carries exactly one of the three presentation classifications and the three counts sum to the published count; every present row's sample id resolves to a receive row; every interpolated present row names the sample id it was derived from; each record type validates against its declared column schema; every timestamp column is present, monotonic and positive; the log records both the in-process process-start value and the `am start -W` output with its launch state on Android; and a run killed at 30 seconds leaves a readable log containing at least 29 seconds of rows in all three record types. Run-to-run percentile stability is deliberately not a gate: p95 tail latency varies with background OS work, compositor behaviour and thermal state by more than any threshold worth writing down.
...
Gate: two parts, separated because one is a property and the other is a distribution. The property is asserted from the per-frame state trace under a frozen scenario: with the reveal enabled, the first-usable-value event occurs no later than the reveal's own first rendered frame plus one frame. That establishes the reveal is drawn over a usable dashboard rather than in front of one, which is what "never delays the first usable value" means, and it is a within-run comparison that background OS work cannot perturb. The distribution is reported, not gated: launch-to-first-usable-value from 4.8's event log across five runs with the reveal and five without, reported as two distributions with their sample counts and spread. Comparing two five-run samples of a cold-start time as pass or fail would be reading noise; the Round 1 phrasing "occurs no later than" across independent runs is withdrawn for that reason.
...
Deliverable: `scripts/capture-metrics.ps1` driving a scenario on a named target and emitting the PROJECT-PLAN section 8 table, with build id, device, resolution, power mode, active RHI, driver string, temperature and sample size in the header. It selects its doctor profile from that target per 1.6. Every run-level statistic is computed here from 4.8's three raw record types over the whole run: frame time p95 and p99 and missed-frame count from the per-frame rows; receive-to-present p95 from the receive and present rows; resident and texture memory trend and temperature from the 1 Hz sampled stream. The harness never copies a rolling percentile out of the overlay.
**Presentation coverage is reported next to latency**, per signal: `presented` divided by published, with the `coalesced` and `pending` counts alongside. Receive-to-present p95 is computed over `presented` samples only, so the table states both how quickly displayed samples reached the screen and what fraction of samples reached it at all. A latency figure without that denominator is easy to improve by displaying less.
Launch timing: on both platforms the harness computes launch-to-first-usable-value from the in-process process-start value and the first-usable-frame event, both in the process's monotonic clock per 4.8. On Android it also records the `am start -W` durations and the reported launch state in the header as auxiliary evidence, and records any disagreement with the in-process figure rather than reconciling it.
...
Gate: the run completes 8 hours without a crash, a hang or a rendering stall longer than 1 second; the sampled stream is continuous with no gap greater than 2 seconds and carries resident memory, texture memory and handle count in every row; and the resident-memory slope over the window from 30 minutes to 8 hours is reported in MB per hour with its confidence interval, alongside the peak and the final value, using the same criterion as 6.6. A crash or a hang before 8 hours is recorded with its elapsed time and the log up to that point, and is a Stage 0 finding rather than a retry until it passes.
...
Gate: a table of decode ms and upload-to-usable ms per asset size on both platforms, with at least 20 samples per cell, every sample traceable to a per-frame row in 4.8's event log carrying the decode and upload-to-usable timestamps for that asset. PROJECT-PLAN section 4 flags this as unvalidated; this task is what validates it.
...
This report is written whether Stage 0 succeeded or stopped. If the stop rule fired at 4.0b or at 6.4, the report is written from the evidence that exists at the stop, and every cell the stop made unreachable is written as **"not measured (blocked by `<task>`)"** with a link to the evidence that produced the stop, such as the committed `logcat`. An empty cell, an omitted row and a target quietly standing in for a measurement are all failures of this gate; "not measured" with a named blocker is not.
Gate: every exit condition in **Goal** above has an explicit met/not-met line with a linked artifact, and every cell of the metrics table is either a measurement traceable to a run or the string "not measured (blocked by `<task>`)" with a named blocking task and a linked artifact.
...
0.5 Turnkey Android   -> 4.0, 4.2 Android/Linux half, 6.2, 6.3
0.6 Linux toolchain   -> 4.2 Android/Linux half, 6.8
...
4.2 Android/Linux half<- 0.5, 0.6
...
7.1  <- 6.10, 4.11, 5.9 | or a recorded stop at 4.0b or 6.4
7.2  <- 7.1
7.3  <- 7.1
7.4  <- 7.1
```

Legend for the one `|` in the block: 7.1 has two alternative predecessor sets and either satisfies it. Normally it waits for 6.10, 4.11 and 5.9. If the stop rule fired at 4.0b (zero working RHIs) or at 6.4 (no matrix cell runs at all), those tasks will never complete, and the recorded stop is itself the predecessor: 7.1 starts from the stop's evidence and writes every unreachable metric as "not measured (blocked by `<task>`)". Every other `<-` in the block is an ordinary all-of dependency. Without that alternative the report documenting a feasibility failure would depend on the tasks the failure prevented.
